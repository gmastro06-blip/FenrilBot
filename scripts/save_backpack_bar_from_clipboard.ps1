...
if ($PSCmdlet.ParameterSetName -eq 'ByName') {
  $out = "src/repositories/inventory/images/containersBars/$Name v2.png"
  $label = $Name
} else {
  $out = "src/repositories/inventory/images/containersBars/$Color Backpack v2.png"
  $label = "$Color Backpack"
}

Write-Host "Copy the $label container bar image to clipboard, then run this script." -ForegroundColor Cyan
